...
@st.cache_resource
def load_visualizer():
    # THE DEFINITIVE FIX IS HERE: Build the model once, then cache the built version.
...
def adjust_hsl(image, hue, sat, bright):
    image_hsv = image.convert("HSV"); h, s, v = image_hsv.split()
...
def main():
    ...
    if uploaded_file:
        ...
        visualizer = load_visualizer(); predictor = load_predictor()
        img_resized_128 = image.resize((128, 128)); img_resized_256 = image.resize((256, 256))
        ...
        last_conv_layer_name = next((l.name for l in reversed(visualizer.layers) if 'conv' in l.name and hasattr(l, 'output') and len(l.output.shape) == 4), None)
        ...
        else: st.warning("Could not find a suitable convolutional layer for Grad-CAM.")
        ...
        with torch.no_grad(): pred_output = predictor(img_tensor)
        ...
        damage_in = quantify_damage(hsl_input_adj); damage_out = quantify_damage(hsl_output_adj)
